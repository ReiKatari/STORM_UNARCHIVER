...
- **`Files\`** — Автономные установщики программы (`STORM_UNARCHIVER_v1.0.0_Setup.exe`) с сохранением всех версий.
...
2. Соберет единый подписанный установщик `.exe` (издатель STORM TEAM) в каталоге `Files\` с ярлыками на Рабочем столе, меню «Пуск» и записями в реестре (версия 1.0.0).
...
2. **Настройка параметров**:
   ...
   - Укажите нужные пароли в менеджере словарей, чтобы программа подбирала их к запароленным архивам.
3. **Закрытие и трей**: Программа работает незаметно! При закрытии окна она всегда сворачивается в трей с обновленным стандартным контекстным меню.
4. **Запуск**: Нажмите **«Начать мониторинг»**. Приложение начнет отслеживать появление файлов и автоматически распаковывать их в фоне.
